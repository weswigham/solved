...
describe("a rainradar solver", () => {
    it("should be able to solve a simple rainradar puzzle", () => {
        ...
        (0, chai_1.expect)(resolved.length).to.equal(1);
        (0, chai_1.expect)(resolved[0]).to.be.deep.equal({ columns: [0, -1, -1], rows: [0, -1, -1], clouds: [{ ul: { x: 0, y: 0 }, lr: { x: 2, y: 2 } }] });
    });
    it("should be able to solve a more complicated rainradar puzzle", () => {
        ...
        const resolved = [...solutions];
        (0, chai_1.expect)(resolved.length).to.equal(1);
        (0, chai_1.expect)(resolved[0]).to.be.deep.equal({ columns: [0, -1, 0, 0, -1], rows: [-1, -1, -1, 0, -1], clouds: [{ ul: { x: 0, y: 0 }, lr: { x: 3, y: 4 } }] });
    });
    it("should be able to verify when there are multiple meaningfully different solutions", () => {
        ...
        const resolved = [...solutions];
        (0, chai_1.expect)(resolved.length).to.equal(2);
        (0, chai_1.expect)(resolved[1]).to.be.deep.equal({ columns: [0, -1, -1, 0, -1], rows: [-1, -1, -1, 0, -1], clouds: [
                { ul: { x: 0, y: 0 }, lr: { x: 1, y: 4 } },
                { ul: { x: 3, y: 0 }, lr: { x: 4, y: 4 } }
            ] });
        (0, chai_1.expect)(resolved[0]).to.be.deep.equal({ columns: [0, -1, -1, 0, -1], rows: [-1, -1, -1, 0, -1], clouds: [{ ul: { x: 0, y: 0 }, lr: { x: 3, y: 4 } }] });
    });
    it("should be able to soulve a multicloud rain radar puzzle", () => {
        ...
        const resolved = [...solutions];
        (0, chai_1.expect)(resolved.length).to.equal(1);
        (0, chai_1.expect)(resolved[0]).to.be.deep.equal({ columns: [-1, 0, -1, 0, -1], rows: [-1, 0, 0, 0, -1], clouds: [
