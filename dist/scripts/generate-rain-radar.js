...
const state = { clouds, rows, columns };
...
function deleteConstraints(state) {
    depth++;
    ...
    for (let i = 0; i < newRows.length; i++) {
        if (newRows[i] === -1)
            continue;
        const newPuzzle = { clouds: [], rows: [...newRows.slice(0, i), -1, ...newRows.slice(i + 1)], columns: newColumns };
        const solver = new solved.RainRadar.Solver();
    ...
    for (let i = 0; i < newColumns.length; i++) {
        if (newColumns[i] === -1)
            continue;
        const newPuzzle = { clouds: [], rows: newRows, columns: [...newColumns.slice(0, i), -1, ...newColumns.slice(i + 1)] };
        const solver = new solved.RainRadar.Solver();
        const solutions = solver.solutions(newPuzzle);
        const firstSolution = solutions.next();
        const secondSolution = solutions.next();
...
//# sourceMappingURL=generate-rain-radar.js.map