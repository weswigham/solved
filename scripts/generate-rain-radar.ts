...
const clouds: solved.RainRadarCloud[] = [
...
const state: solved.RainRadarState = {clouds, rows, columns};

const solver = new solved.RainRadarSolver();
let depth = 0;
...
function countConstraints(state: solved.RainRadarState): number {
    return state.rows.reduce((prev, current) => current !== -1 ? prev + 1 : prev, 0) +
        state.columns.reduce((prev, current) => current !== -1 ? prev + 1 : prev, 0);
}


function deleteConstraints(state: solved.RainRadarState): solved.RainRadarState | undefined {
    depth++;
    ...
    for (let i=0; i<newRows.length; i++) {
        ...
        const newPuzzle: solved.RainRadarState = {clouds: [], rows: [...newRows.slice(0, i), -1, ...newRows.slice(i + 1)], columns: newColumns};
        const solver = new solved.RainRadarSolver();
    ...
    for (let i=0; i<newColumns.length; i++) {
        ...
        const newPuzzle: solved.RainRadarState = {clouds: [], rows: newRows, columns: [...newColumns.slice(0, i), -1, ...newColumns.slice(i + 1)]};
        const solver = new solved.RainRadarSolver();
        const solutions = solver.solutions(newPuzzle);
        const firstSolution = solutions.next();
        const secondSolution = solutions.next();
